...
```hcl
module "ecs_process_scheduler" {
  source                             = "git::https://git@github.com/ucopacme/terraform-aws-ecs-fargate.git//?ref=v0.0.19"
  name                               = join("-", [local.application, local.environment, "process-scheduler"])
  enable_autoscaling                 = false
  enable_ecs_cluster                 = true
  managed_ec2_instances              = { PUBSUB = {cpu_max = "2", mem_max = "4096" , storage_gb = "50" } }
  cluster_arn                        = module.ecs_process_scheduler.cluster_arn
  cluster_name                       = module.ecs_process_scheduler.cluster_name
  containerInsights                  = false
  image                              = module.ecr_process_scheduler.repository_url
  memory                             = 512
  cpu                                = 256
  desired_count                      = 1
  ...
  target_group_arn                   = local.ps_target_group_arns[0]
  subnets                            = [local.private_subnet_ids[0], local.private_subnet_ids[1]]
  security_groups                    = [module.sg_process_scheduler.id]
  container_port                     = 8080
...
| [aws_iam_role_policy.ecs_infrastructure_pass_role](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/iam_role_policy) | resource |
...
| [aws_iam_role_policy_attachment.ecs_infrastructure_role_policy_attachment](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/iam_role_policy_attachment) | resource |
| [aws_iam_role_policy_attachment.ecs_instance_role_policy_attachment](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/iam_role_policy_attachment) | resource |
...
| <a name="input_linux_parameters"></a> [linux\_parameters](#input\_linux\_parameters) | Linux-specific modifications that are applied to the container, such as Linux kernel capabilities. For more details, see https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_LinuxParameters.html | <pre>object({<br>    initProcessEnabled = bool<br>  })</pre> | `null` | no |
| <a name="input_managed_ec2_instances"></a> [managed\_ec2\_instances](#input\_managed\_ec2\_instances) | List of managed EC2 instance configurations for ECS capacity providers. cpu\_max (e.g. '2') and mem\_max (e.g. '4096') define the maximum vCPU and memory for instance requirements. | <pre>map(object({<br>    cpu_max    = string<br>    mem_max    = string<br>    storage_gb = string<br>  }))</pre> | `{}` | no |
...
| <a name="input_secrets"></a> [secrets](#input\_secrets) | Hash of name/SecretsManagerARN pairs to include in the task definition as environment variables (see also https://docs.aws.amazon.com/AmazonECS/latest/developerguide/secrets-envvar-secrets-manager.html) | <pre>list(object({<br>    name      = string<br>    valueFrom = string<br>  }))</pre> | `[]` | no |
...
| <a name="input_systemControls"></a> [systemControls](#input\_systemControls) | Hash of name/SecretsManagerARN pairs to include in the task definition as environment variables (see also https://docs.aws.amazon.com/AmazonECS/latest/developerguide/secrets-envvar-secrets-manager.html) | <pre>list(object({<br>    namespace = string<br>    value     = string<br>  }))</pre> | `[]` | no |
